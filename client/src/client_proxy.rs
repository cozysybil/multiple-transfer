...
impl ClientProxy {
    ...
    /// +++ Multi Transfers coins from sender to receivers.
    pub fn multi_transfer_coins(
        &mut self,
        space_delim_strings: &[&str],
        is_blocking: bool,
    ) -> Result<IndexAndSequence> {
        ensure!(
            space_delim_strings.len() >= 7 && space_delim_strings.len() <= 15,
            "Invalid number of arguments for transfer"
        );

        let sender_account_address = self.get_account_address_from_parameter(space_delim_strings[1])?;
        
        let total_coins = Self::convert_to_micro_libras(space_delim_strings[2])?;
        ...
        let number_of_recievers = space_delim_strings[5].parse::<u64>().map_err(|error| {
            format_parse_data_error(
                "number_of_recievers",
                InputType::UnsignedInt,
                space_delim_strings[5],
        ...
        let num_coins = total_coins / number_of_recievers ;

        let sender_account_ref_id = self.get_account_ref_id(&sender_account_address)?;
        for n in 6..space_delim_strings.len()-1 {
            let receiver_address = self.get_account_address_from_parameter(space_delim_strings[n])?;
            let result = self.transfer_coins_int(
                sender_account_ref_id,
                &receiver_address,
                num_coins,
                gas_unit_price,
            ...
            if success != true {
                println!("cannot transfer to {}", space_delim_strings[n]);
            }
        };
        let receiver_address = self.get_account_address_from_parameter(space_delim_strings[space_delim_strings.len()-1])?;
        self.transfer_coins_int(
            sender_account_ref_id,
            &receiver_address,
            num_coins,
